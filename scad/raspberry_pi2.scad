...
pi_base_width  = pi_width + 2 * pi_clearance + 2 * min_wall + 1.5;
...
rpi2_x_holes_location = 24.5 + 1.5/2;
rpi2_y_holes_location1 = 39;
rpi2_y_holes_location2 = 19;

module pi_holes2() {
  for(x=[-1,1],y=[-1,1]) 
    if (y==-1)
      translate([rpi2_x_holes_location*x,rpi2_y_holes_location1*y,0])
        children();
    else
      translate([rpi2_x_holes_location*x,rpi2_y_holes_location2*y,0])
        children();
}
    ...
    union() {
      difference() {
        ...
        translate([0, 0, rim_height / 2 + pi_base])
            cube([pi_base_width - 2 * rim_width +1.5, pi_base_length - 2 * rim_width, rim_height], center = true);

        translate([0, 0, rim_height])
            cube([pi_width + 2 * pi_clearance +1.5, pi_length + 2 * pi_clearance, pi_thickness * 2], center = true);
      ...
      union() {
	    ...
	    pi_holes2() {
		    nut_trap(M2p5_clearance_radius, nut_radius(M2p5_nut), nut_depth, supported = true);
				translate([0,0,-0.1])
					cylinder(r=1.5,h=pi_lift*2,$fn=8);
			}
		}
